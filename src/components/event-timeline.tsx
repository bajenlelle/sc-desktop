    ...
    <div className="space-y-1">
      {events.map((event, i) => (
        <div
          key={event.id}
          className={cn(
            "group flex items-center gap-4 rounded-lg px-4 py-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800",
            i % 2 === 0 ? "bg-white dark:bg-slate-900" : "bg-slate-50/50 dark:bg-slate-800/50"
          )}
        >
          {/* Timestamp */}
          <div className="flex w-16 shrink-0 items-center justify-center">
            <span className="rounded-md bg-slate-100 dark:bg-slate-800 px-2 py-1 font-mono text-xs font-semibold text-slate-600 dark:text-slate-400">
              {event.timestamp}
            </span>
          </div>
          ...
          <span
            className={cn(
              ...
              outcomeStyles[event.outcome] ?? "text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800"
            )}
          >
