 ...
 * profile (for org_id), and only the secondary-org team read depends on
 * myOrgs. team_members and get_my_orgs need nothing but the uid, so they ride
 * in the first wave rather than trailing it.
 */
export async function getOrgContext(): Promise<OrgContext> {
  ...
  const [orgRes, teamsRes, membersRes, secondaryTeamsRes] = await Promise.all([
    primaryOrgId
      ? supabase
          .from("organizations")
  ...
  const secondaryOrgTeams: OrgTeam[] = secondaryTeamsRes?.data
    ? (secondaryTeamsRes.data as TeamRow[]).map(rowToTeam)
    : [];
 ...
 * run as two extra serial waves, making this four round trips deep. The
 * aggregated player feed calls this once per club, so the depth multiplied.
 *
 * Pass `myOrgs` when the caller already has it (the auth context resolves it
 * at sign-in). Callers mapping over several orgs should always pass it — the
 * result is identical for every org.
 */
...
export async function createOrgForPlatform(input: CreateOrgInput): Promise<string> {
  const supabase = createClient();
  const { data, error } = await supabase.rpc("create_org_for_platform", {
    org_name: input.name,
    p_is_nt_org: input.isNtOrg ?? false,
    p_plan_tier: input.planTier ?? "franchise",
    p_coach_seats: input.coachSeats ?? null,
    p_player_seats: input.playerSeats ?? null,
...
export async function getOrgById(orgId: string): Promise<Organization> {
  ...
  const { data, error } = await supabase
    .from("organizations")
    .select("id, name, logo_url, created_at, coach_seat_limit, player_seat_limit, expires_at, is_nt_org, plan_tier, contact_name, contact_email, notes")
    .eq("id", orgId)
    .single();
...
export async function updateOrgLicense(
  ...
  if (error) {
    if (error.message.includes("not_platform_admin")) throw new Error("Not authorized as platform admin.");
    if (error.message.includes("invalid_seat_limit")) throw new Error("Seat limits can't be negative.");
...
export async function updateOrgContact(
  ...
  const supabase = createClient();
  const { error } = await supabase.rpc("update_org_contact", {
    p_org_id: orgId,
...
export async function listOrgLicenseEvents(orgId: string): Promise<OrgLicenseEvent[]> {
  const supabase = createClient();
  const { data, error } = await supabase.rpc("list_org_license_events", { p_org_id: orgId });
  if (error) {
    if (error.message.includes("not_platform_admin")) throw new Error("Not authorized as platform admin.");
    throw new Error(`Failed to load license history: ${error.message}`);
  }
  return (data ?? []).map((r: {
    id: number;
    event: string;
    ...
    id: r.id,
    event: r.event as OrgLicenseEvent["event"],
