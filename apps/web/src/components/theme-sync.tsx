...
/**
 * Cross-device theme sync watcher — web twin of desktop's
 * apps/desktop/src/components/theme-sync.tsx (keep in sync; deltas: auth
 * context and supabase client import paths). Renders nothing. Mounted inside
 * the (app) AuthProvider tree — the root layout has no auth context, and
 * signed-out pages need no sync.
 *
 ...
 * snapshot exists. Failed writes revert the ref. The realtime socket is
 * best-effort: focus-triggered profile reloads remain the catch-up path.
 */
