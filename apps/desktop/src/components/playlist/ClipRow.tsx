...
export interface ClipRowProps {
  ...
  note?: string;
  /** False when the clip has no exported cloud file and so can't be played. */
  playable: boolean;
  watched: boolean;
  active: boolean;
  onSelect: () => void;
...
/**
 * One clip in a player's shared playlist.
 *
 * Recipients don't have the coach's local footage, so playback depends on the
 * clip having been exported to cloud storage by Clip & Ship. Clips without one
 * are shown but disabled rather than hidden, so the playlist still reads as
 * the coach assembled it.
 */
export function ClipRow({
  ...
  return (
    <button
      ...
      disabled={!playable}
      title={playable ? undefined : "Still uploading — not available to watch yet"}
      className={cn(
        "flex w-full items-stretch gap-0 text-left transition-colors",
        playable ? "hover:bg-muted/50" : "opacity-50 cursor-not-allowed",
